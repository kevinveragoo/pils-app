...
const initialEnrollment = (): Values => ({ enrollment_date_11a07b: today(), implementing_partner_41a2cd: "PILS", district_979c1b: "", hotspot: "1", client_outreach_worker: "", client_last_name: "", client_first_name: "", client_middle_name: "", client_alias: "", client_dob: "", client_gender_identity: "", client_kp_type: "", phone_primary: "", preferred_contact_method: "", risk_drug_alcohol_sex: "", risk_violence_month: "", sw_age_started: "", sw_sex_acts_week: "", sw_condom_intimate: "", msm_age_first_anal: "", msm_receptive_anal_week: "", msm_condom_anal: "", pwid_age_first_inject: "", pwid_injections_24h_b12f8a: "", pwid_injections_week: "", pwid_shared_24h: "", pwid_shared_week: "" });
...
const initialMonitoring = (): Values => ({ hiv_monitor_date: today(), art_status: "", cd4_date: "", cd4_level: "", vl_date: "", vl_level: "", hiv_monitor_notes: "" });
...
export default function HealthcareNavWorkflow() {
  ...
  const [uicCheck, setUicCheck] = useState<"idle" | "available" | "exists" | "error">("idle");
  ...
  const setE = (key: string, value: string) => { setError(""); if (["client_first_name", "client_middle_name", "client_last_name", "client_dob", "client_gender_identity"].includes(key)) setUicCheck("idle"); setEnrollment((current) => ({ ...current, [key]: value })); };
  ...
  useEffect(() => {
    ...
    fetch(`/api/redcap/healthcare-nav?uic=${encodeURIComponent(uic)}`, { signal: controller.signal }).then(async (response) => { const data = await response.json(); if (!response.ok) throw new Error(data.error); setUicCheck(data.exists ? "exists" : "available"); }).catch((reason: unknown) => { if ((reason as { name?: string }).name !== "AbortError") setUicCheck("error"); });
  ...
  function validateCurrent() {
    setError("");
    if (step === 1 && !mode) return setError("Choose whether this is a new or existing client."), false;
    ...
    if (step === 3) {
      ...
      if (missing.length) return setError(`Still required: ${missing.join(", ")}.`), false;
      if (care.hc_followup_needed === "1" && !care.hc_followup_date) return setError("Enter the planned follow-up date."), false;
    }
    if (step === 4 && (newCd4 || newVl)) {
      const missing = [!monitoring.hiv_monitor_date && "Monitoring date", newCd4 && !monitoring.cd4_date && "CD4 test date", newCd4 && !monitoring.cd4_level && "CD4 level", newVl && !monitoring.vl_date && "Viral-load test date", newVl && !monitoring.vl_level && "Viral-load level"].filter(Boolean);
      if (missing.length) return setError(`Still required: ${missing.join(", ")}.`), false;
    }
    return true;
  }
  ...
  return <div className="outreach-page mx-auto w-full max-w-4xl px-4 sm:px-6">
    ...
    <ol className="mb-6 flex items-center gap-2 overflow-x-auto pb-1" aria-label="Progress">{["Client", "Enrollment", "HIV care", "Monitoring", "Review"].map((label, index) => <li key={label} className={`flex items-center gap-2 whitespace-nowrap text-sm font-bold ${step === index + 1 ? "text-primary" : "text-muted-foreground"}`}><span className="outreach-step">{index + 1}</span>{label}{index < 4 && <span aria-hidden className="mx-1 h-px w-5 bg-border" />}</li>)}</ol>
    ...
    {step === 1 && <ClientChoice mode={mode} setMode={(value) => { setError(""); setMode(value); }} />}
    {step === 2 && mode === "existing" && <ClientSearch query={query} setQuery={setQuery} loading={loadingPatients} matches={matches} selected={selected} setSelected={(patient) => { setError(""); setSelected(patient); }} />}
    ...
    <div className="mt-5 flex justify-between gap-3"><Button variant="outline" disabled={step === 1 || submitting} onClick={() => { setError(""); setStep((current) => current - 1); }}><ArrowLeft />Back</Button>{step < 5 ? <Button disabled={step === 2 && mode === "new" && uicCheck === "exists"} onClick={goForward}>Continue<ArrowRight /></Button> : <Button disabled={submitting} onClick={submit}>{submitting ? "Saving…" : "Submit to REDCap"}<Check /></Button>}</div>
  </div>;
}

function ClientChoice({ mode, setMode }: { mode: Mode | null; setMode: (mode: Mode) => void }) {
  return <section className="outreach-card"><h2 className="text-xl font-bold">Is this a new client?</h2><p className="mb-5 text-sm text-muted-foreground">New clients complete a short enrollment before HIV care support.</p><div className="grid gap-3 sm:grid-cols-2"><button onClick={() => setMode("new")} className={`rounded-2xl border p-5 text-left ${mode === "new" ? "border-primary bg-secondary" : ""}`}><strong>New client</strong><span className="mt-1 block text-sm text-muted-foreground">Create enrollment and UIC</span></button><button onClick={() => setMode("existing")} className={`rounded-2xl border p-5 text-left ${mode === "existing" ? "border-primary bg-secondary" : ""}`}><strong>Existing client</strong><span className="mt-1 block text-sm text-muted-foreground">Find by UIC, name, or alias</span></button></div></section>;
}

function ClientSearch({ query, setQuery, loading, matches, selected, setSelected }: { query: string; setQuery: (value: string) => void; loading: boolean; matches: Patient[]; selected: Patient | null; setSelected: (patient: Patient) => void }) {
  return <section className="outreach-card"><h2 className="text-xl font-bold">Find client</h2><div className="relative mt-4"><Search className="absolute left-3 top-3.5 size-5 text-muted-foreground" /><Input className="pl-10" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="UIC starts with… or name includes…" /></div>{loading ? <p className="mt-4 text-sm text-muted-foreground">Loading clients…</p> : <div className="mt-3 grid gap-2">{matches.map((patient) => <button key={patient.uic} onClick={() => setSelected(patient)} className={`rounded-xl border p-3 text-left ${selected?.uic === patient.uic ? "border-primary bg-secondary" : ""}`}><strong>{patient.display_name}</strong><span className="block text-sm text-muted-foreground">{patient.uic}{patient.alias ? ` · ${patient.alias}` : ""}</span></button>)}</div>}</section>;
}
...
function Enrollment({ values: v, set, uic, kp }: { values: Values; set: (key: string, value: string) => void; uic: string; kp: Set<string> }) {
  ...
  return <section className="outreach-card grid gap-6"><div><h2 className="text-xl font-bold">Client enrollment</h2><p className="text-sm text-muted-foreground">Required identity, contact, and initial risk information.</p></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Enrollment date" required><DateInput value={v.enrollment_date_11a07b} onChange={(value) => set("enrollment_date_11a07b", value)} /></Field><Field label="Outreach worker" required><Input value={v.client_outreach_worker} onChange={(event) => set("client_outreach_worker", event.target.value)} /></Field><Field label="First name" required><Input value={v.client_first_name} onChange={(event) => set("client_first_name", event.target.value)} /></Field><Field label="Middle name(s)"><Input value={v.client_middle_name} onChange={(event) => set("client_middle_name", event.target.value)} /></Field><Field label="Surname" required><Input value={v.client_last_name} onChange={(event) => set("client_last_name", event.target.value)} /></Field><Field label="Alias"><Input value={v.client_alias} onChange={(event) => set("client_alias", event.target.value)} /></Field><Field label="Date of birth" required><DateInput value={v.client_dob} onChange={(value) => set("client_dob", value)} /></Field><Field label="Gender identity" required><Select value={v.client_gender_identity} onChange={(value) => set("client_gender_identity", value)}><option value="1">Man</option><option value="2">Woman</option><option value="3">Transgender man</option><option value="4">Transgender woman</option><option value="5">Other</option><option value="9">Refuse to answer</option></Select></Field><Field label="District" required><Select value={v.district_979c1b} onChange={(value) => set("district_979c1b", value)}>{["Port Louis", "Pamplemousses", "Rivière du Rempart", "Flacq", "Grand Port", "Savanne", "Plaines Wilhems", "Moka", "Black River"].map((label, index) => <option key={label} value={String(index + 1)}>{label}</option>)}<option value="99">Other</option></Select></Field><Field label="Primary phone"><Input type="tel" value={v.phone_primary} onChange={(event) => set("phone_primary", event.target.value)} /></Field></div><Field label="Key population" required><ChoiceChecks values={v.client_kp_type} onChange={updateKp} options={[["1", "Sex worker"], ["2", "MSM"], ["3", "PWID"], ["4", "PPWID"], ["5", "Transgender"], ["6", "Former PWID"], ["7", "General"]]} /></Field>{uic && <div className="rounded-2xl bg-secondary p-4"><span className="text-xs font-bold uppercase tracking-wider text-secondary-foreground">Generated UIC</span><strong className="mt-1 block text-xl tracking-wide">{uic}</strong></div>}<details className="rounded-2xl border p-4"><summary className="cursor-pointer font-bold">Initial risk snapshot</summary><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="Drugs/alcohol during sex last week"><YesNo value={v.risk_drug_alcohol_sex} onChange={(value) => set("risk_drug_alcohol_sex", value)} /></Field><Field label="Violence in the last month"><YesNo value={v.risk_violence_month} onChange={(value) => set("risk_violence_month", value)} /></Field>{kp.has("1") && <><Field label="Age when sex work began"><Input type="number" min="0" value={v.sw_age_started} onChange={(event) => set("sw_age_started", event.target.value)} /></Field><Field label="Sex acts last week"><Input type="number" min="0" value={v.sw_sex_acts_week} onChange={(event) => set("sw_sex_acts_week", event.target.value)} /></Field><Field label="Condom every time with intimate partner"><YesNo value={v.sw_condom_intimate} onChange={(value) => set("sw_condom_intimate", value)} /></Field></>}{(kp.has("2") || kp.has("5")) && <><Field label="Age at first anal sex"><Input type="number" min="0" value={v.msm_age_first_anal} onChange={(event) => set("msm_age_first_anal", event.target.value)} /></Field><Field label="Receptive anal sex acts last week"><Input type="number" min="0" value={v.msm_receptive_anal_week} onChange={(event) => set("msm_receptive_anal_week", event.target.value)} /></Field><Field label="Condom every time during anal sex"><YesNo value={v.msm_condom_anal} onChange={(value) => set("msm_condom_anal", value)} /></Field></>}{(kp.has("3") || kp.has("4")) && <><Field label="Age at first injection"><Input type="number" min="0" value={v.pwid_age_first_inject} onChange={(event) => set("pwid_age_first_inject", event.target.value)} /></Field><Field label="Injections in the last 24 hours"><Input type="number" min="0" value={v.pwid_injections_24h_b12f8a} onChange={(event) => set("pwid_injections_24h_b12f8a", event.target.value)} /></Field><Field label="Injections last week"><Input type="number" min="0" value={v.pwid_injections_week} onChange={(event) => set("pwid_injections_week", event.target.value)} /></Field><Field label="Shared equipment in the last 24 hours"><Input type="number" min="0" value={v.pwid_shared_24h} onChange={(event) => set("pwid_shared_24h", event.target.value)} /></Field><Field label="Shared equipment last week"><Input type="number" min="0" value={v.pwid_shared_week} onChange={(event) => set("pwid_shared_week", event.target.value)} /></Field></>}</div></details></section>;
}

function CareSupport({ values: v, set, patient }: { values: Values; set: (key: string, value: string) => void; patient: Patient | null }) {
  return <section className="outreach-card grid gap-5"><div><h2 className="text-xl font-bold">HIV Care Support</h2>{patient && <p className="text-sm text-muted-foreground">For {patient.display_name} · {patient.uic}</p>}</div><details open className="rounded-2xl border p-4"><summary className="cursor-pointer font-bold">HIV CARE &amp; SUPPORT DETAILS</summary><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="Healthcare navigator" required><Input value={v.hiv_care_navigator} onChange={(event) => set("hiv_care_navigator", event.target.value)} /></Field><Field label="Healthcare navigator 2"><Input value={v.hiv_care_navigator_2} onChange={(event) => set("hiv_care_navigator_2", event.target.value)} /></Field><Field label="Healthcare navigator / driver"><Input value={v.hiv_care_navigator_3} onChange={(event) => set("hiv_care_navigator_3", event.target.value)} /></Field><Field label="Date of contact" required><DateInput value={v.hiv_care_date} onChange={(value) => set("hiv_care_date", value)} /></Field><Field label="Currently on ART?" required><YesNo value={v.hiv_currently_art} onChange={(value) => set("hiv_currently_art", value)} /></Field><Field label="HIV/ART status" required><Select value={v.hiv_art_status_code} onChange={(value) => set("hiv_art_status_code", value)}><option value="1">Newly diagnosed</option><option value="2">Known HIV positive</option><option value="3">Lost to follow-up</option></Select></Field></div></details><details open className="rounded-2xl border p-4"><summary className="cursor-pointer font-bold">ACCOMPANIMENT DETAILS</summary><div className="mt-4 grid gap-4"><Field label="Accompaniment type"><ChoiceChecks values={v.hiv_care_type} onChange={(value) => set("hiv_care_type", value)} options={[["1", "Social Security Board"], ["2", "Eye Hospital"], ["3", "Orthopaedic Hospital"], ["4", "DCCI"], ["5", "Banian"], ["6", "Methadone Centre"], ["7", "Hospital visit"], ["8", "Home visit"]]} /></Field><Field label="Accompaniment region"><Select value={v.hiv_care_region} onChange={(value) => set("hiv_care_region", value)}><option value="1">Jeetoo</option><option value="3">Candos</option><option value="4">SSRN</option><option value="5">Flacq</option><option value="6">Beau Bassin</option><option value="7">Port Louis</option></Select></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="ART adherence counselling provided?"><YesNo value={v.hiv_adherence_counsel} onChange={(value) => set("hiv_adherence_counsel", value)} /></Field><Field label="Psychosocial support/referrals provided?"><YesNo value={v.hiv_psychosocial} onChange={(value) => set("hiv_psychosocial", value)} /></Field><Field label="Referral made to comprehensive HIV care?"><YesNo value={v.hiv_comprehensive_ref} onChange={(value) => set("hiv_comprehensive_ref", value)} /></Field></div></div></details><details className="rounded-2xl border p-4"><summary className="cursor-pointer font-bold">COMMODITIES PROVIDED</summary><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="Male condoms"><Input type="number" min="0" value={v.hiv_male_condoms} onChange={(event) => set("hiv_male_condoms", event.target.value)} /></Field><Field label="Female condoms"><Input type="number" min="0" value={v.hiv_female_condoms} onChange={(event) => set("hiv_female_condoms", event.target.value)} /></Field><Field label="Lubricant"><Input type="number" min="0" value={v.hiv_lube} onChange={(event) => set("hiv_lube", event.target.value)} /></Field><Field label="Needles/syringes"><Input type="number" min="0" value={v.hiv_needles} onChange={(event) => set("hiv_needles", event.target.value)} /></Field></div></details><details open className="rounded-2xl border p-4"><summary className="cursor-pointer font-bold">FOLLOW-UP</summary><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="Follow-up required?"><YesNo value={v.hc_followup_needed} onChange={(value) => { set("hc_followup_needed", value); if (value !== "1") set("hc_followup_date", ""); }} /></Field>{v.hc_followup_needed === "1" && <Field label="Planned follow-up date" required><DateInput value={v.hc_followup_date} onChange={(value) => set("hc_followup_date", value)} /></Field>}</div><div className="mt-4"><Field label="Healthcare navigator notes"><textarea value={v.hc_notes} onChange={(event) => set("hc_notes", event.target.value)} /></Field></div></details></section>;
}
